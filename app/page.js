    ...
    <div>
      {!fileName ? (
        <div onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
          ...
          <input ref={inputRef} type="file" accept=".txt,.md,.csv" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (f) readFile(f); }} />
          {processing ? <p style={{ color: "#2E75B6", fontSize: 13, margin: 0, fontFamily: "'JetBrains Mono', monospace" }}>Procesando...</p>
            : <><p style={{ color: "#888", fontSize: 24, margin: "0 0 8px" }}>📄</p><p style={{ color: "#666", fontSize: 13, margin: "0 0 4px", fontFamily: "'JetBrains Mono', monospace" }}>Arrastra o haz clic</p><p style={{ color: "#444", fontSize: 11, margin: 0 }}>.txt .md .csv</p></>}
        </div>
      ) : (
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "40px 20px" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          {inputMode === "text" ? (
            ...
            <FileUpload onFileContent={(t, n) => { setDeliverable(t); setFileName(n); }} fileName={fileName} onClear={clearFile} />
          )}
        </div>
